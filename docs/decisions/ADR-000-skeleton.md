...
| **Status** | DECIDED + IMPLEMENTED — 2026-07-09. Adopting full PLUTO as canonical substrate. Created `watchline/shared/buildings.py` with `load_pluto()` and `load_backfill()` functions shared by both graphs. Discovery's `buildings/pipeline.py` now delegates to the shared module. New `watchline/evidentiary/ingest/buildings/pipeline.py` created; `evidentiary-buildings` Makefile target added as a prerequisite for all event pipelines. Phase 4 follow-up (2026-07-09): migrated all four evidentiary event pipelines (`hpd_violations`, `dob_violations`, `ecb_violations`, `hpd_litigations`) from fat PLUTO-joined building-creation SQL to schema-only MERGE stubs. Each stub now does a `SELECT DISTINCT bbl` scan of its source table and a minimal MERGE that only sets `borough` if null — no PLUTO enrichment (that is owned by the shared substrate). Removed `BUILDINGS_SQL`, `_building_batches()`, the PLUTO LEFT JOIN, and all property enrichment Cypher from each pipeline. Removed the now-unused `BUILDING_BATCH_SIZE` alias from all four. |
...
| **Status** | DECIDED + IMPLEMENTED — 2026-07-09. Live query confirmed: `isndobbisviol` is unique (2,475,392 distinct = 2,475,392 total rows, zero nulls). `number` has 8,601 duplicates — it is not a primary key. Evidentiary `dob_violations/pipeline.py` already uses `EVT-DOB-{isndobbisviol}` (line 346) and documents `(ADR-003)` in its docstring — the code was correct when reviewed. No Python change needed. Operational note: any live evidentiary graph built before this ADR was recorded may contain orphaned `EVT-DOB-{number}` nodes; a full graph rebuild (`make evidentiary-build` on a wiped database) clears them. The IS KEY constraint prevents in-place rename. |
...
## ADR-015 — Layer 5 (InvestigativeIntent, CanonicalQuestion) specified but never populated or consulted

| | |
|---|---|
| **Topic** | `documents/ontology-implementable.md` and `scripts/01_schema.cypher` define `InvestigativeIntent` and `CanonicalQuestion` node types (Layer 5, Investigation Nodes) from day one, but as of 2026-07-16 zero nodes of either type existed in the live evidentiary graph, and no code path in `watchline/fw/` ever read them. Intent classification in `watchline/fw/intent.py::identify_intent` is a single LLM call whose only anchor is a hardcoded category list inside `INTENT_SYSTEM_PROMPT`; `watchline/fw/router.py::select_rules` then does a plain `REGISTRY.get(intent_cat)` dict lookup with no check against anything in the graph. |
| **Evidentiary approach** | N/A — this is evidentiary-only; discovery has no `fw/` query layer (see ADR-012). |
| **Discovery approach** | N/A |
| **Principle** | Charter Principle 17 (Deterministic AI Orchestration): "Every conditional transition must be justified by a defined Rule or Canonical Question in the ontology." The intent-classification transition — the first and most consequential routing decision in the whole pipeline — was justified by neither; also P-11 (leave the graphs agent-ready). |
| **Recommended resolution** | (a) Seed one `InvestigativeIntent` node per real entry in `watchline/fw/intents/__init__.py::REGISTRY` (13; `General` excluded — it is a catch-all fallback, not a named investigative question) plus at least one `CanonicalQuestion` per intent, using tested real questions as templates rather than inventing new ones. (b) Add a validation gate in `select_rules` that checks the LLM's `intent_category` against a real `InvestigativeIntent.name` before trusting it, carrying the matched `intent_id` through `traversal_results` as `justified_by_intent_id` so the routing decision is citable, not just internally checked. Do not restructure classification to run primarily off `CanonicalQuestion.question_template` matching — that's a larger architecture change than Principle 17 requires; the LLM still classifies, but its output is now checked against the ontology instead of trusted unconditionally. |
| **Status** | DECIDED + IMPLEMENTED — 2026-07-16. Created `scripts/04_seed_investigative_intents.cypher`: 13 `InvestigativeIntent` nodes (`INT-00001`–`INT-00013`, one per `REGISTRY` entry, `description` text drawn verbatim from `INTENT_SYSTEM_PROMPT`'s existing one-liners) and 15 `CanonicalQuestion` nodes (`CQ-00001`–`CQ-00015`; `EnforcementAccountability` and `OwnershipChange` each got two, matching two distinct phrasings that were actually tested against those handlers). `applicable_rules` values were verified against each handler's `_GRAPH_RULE_ID` constant by grep, not copied from CLAUDE.md's Rule table, which is stale (lists `RUL-00009` as the highest rule_id; `RUL-00010`–`RUL-00014` already exist in the handler code — CLAUDE.md needs a separate follow-up correction, not done as part of this ADR). Ran against the live evidentiary database via the Neo4j MCP connector: 13 `InvestigativeIntent` + 15 `CanonicalQuestion` nodes created, zero orphaned `CanonicalQuestion.intent_id` references, all 13 `REGISTRY` keys confirmed to exact-match a seeded `InvestigativeIntent.name`. Added `_validate_intent()` and wired `justified_by_intent_id` into all four `traversal_results`-constructing branches of `select_rules` in `watchline/fw/router.py`. Added `evidentiary-seed-intents` Makefile target (grouped with `evidentiary-seed-rules` in `evidentiary-build`'s dependency chain) and three sanity checks to `evidentiary-verify`. Not yet done, flagged as follow-up: `narrator.py`'s `PRESENT_SYSTEM_PROMPT` does not yet instruct the LLM to cite `justified_by_intent_id` in the narrated answer — the data is present in the context passed to the narrator (confirmed by reading `narrator.py`'s generic `handler`-only key-strip) but not yet surfaced to end users. Also not done: `ownership_change.py` is fully implemented and registered despite CLAUDE.md stating `OwnershipChange` should remain a stub pending ACRIS history — seeded its `InvestigativeIntent` node anyway since `REGISTRY` already exposes it, but this CLAUDE.md/code discrepancy should be reconciled separately. |
