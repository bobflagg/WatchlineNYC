...
Key differences from HPD violations pipeline:
  - Primary key is the 'number' field (not a numeric id)
...
Usage:
    uv run python -m watchline.ingest.dob_violations.pipeline
    uv run python -m watchline.ingest.dob_violations.pipeline --step source
    uv run python -m watchline.ingest.dob_violations.pipeline --step buildings
    uv run python -m watchline.ingest.dob_violations.pipeline --step violations
"""
...
def _building_batches(conn) -> Iterator[List[dict]]:
    ...
    with conn.cursor(cursor_factory=RealDictCursor) as cur:
        ...
        for row in cur:
            ...
            borough_code = bbl[0] if bbl else "0"
            borough = BOROUGH_CODE_MAP.get(borough_code, "Unknown")
...
def _violation_batches(conn) -> Iterator[List[dict]]:
    ...
    with conn.cursor(name="dob_violations_cursor",
        ...
        for row in cur:
            ...
            batch.append({
                "event_id":              f"EVT-DOB-{row['number']}",
                "bbl":                   row["bbl_canonical"],
                "source_record_id":      str(row["number"]),
