...
Usage:
    uv run python -m watchline.ingest.rentstab.pipeline
    uv run python -m watchline.ingest.rentstab.pipeline --step source
    uv run python -m watchline.ingest.rentstab.pipeline --step enrich
"""
...
def _batches(conn) -> Iterator[List[dict]]:
    with conn.cursor(cursor_factory=RealDictCursor) as cur:
        ...
        for row in cur:
            ...
            borough = BOROUGH_CODE_MAP.get(bbl[0], "Unknown")
