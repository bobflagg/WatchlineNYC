...
Usage:
    uv run python -m watchline.ingest.ecb_violations.pipeline
    uv run python -m watchline.ingest.ecb_violations.pipeline --step source
    uv run python -m watchline.ingest.ecb_violations.pipeline --step buildings
    uv run python -m watchline.ingest.ecb_violations.pipeline --step violations
"""
...
SELECT
    ...
    MAX(e.respondenthousenumber) AS housenumber,
    MAX(e.respondentstreet)      AS streetname,
...
def _building_batches(conn) -> Iterator[List[dict]]:
    with conn.cursor(cursor_factory=RealDictCursor) as cur:
        print("  Querying distinct buildings from ecb_violations + pluto_latest ...")
        cur.execute(BUILDINGS_SQL)
        batch = []
        for row in cur:
            bbl = row["bbl_canonical"]
            borough = BOROUGH_CODE_MAP.get(bbl[0] if bbl else "0", "Unknown")
...
def load_buildings(session, conn) -> int:
    """MERGE Building nodes -- only fills in null properties to preserve HPD/DOB data."""
    cypher = """
    UNWIND $batch AS b
