...
`Landlord` secondary label, not `Actor` -- since acris_deeds, `Actor`
...
def project_graph(gds: GraphDataScience):
    """
    Project Landlord nodes + both connection edge types as an
    undirected weighted graph.

    Projects the `Landlord` secondary label, NOT `Actor`. Since
    ...
    off the node store, matching pre-acris_deeds memory/perf. `Landlord`
...
def _get_bbl_counts(session: Session) -> Dict[int, int]:
    ...
    associated with. Scoped to :Landlord (see project_graph) --
    ...
    """
    result = session.run(
        "MATCH (a:Landlord) RETURN id(a) AS nodeId, size(a.bbls) AS bblCount"
    )
