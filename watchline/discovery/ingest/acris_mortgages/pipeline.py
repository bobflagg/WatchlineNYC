...
completed at a few-thousand-document sample scale. step_references now
ensures an index on real_property_master(crfn) up front (see
_ensure_crfn_index / CRFN_INDEX_SQL), which turns each lookup into an index
scan; the ~30s one-time build is idempotent (IF NOT EXISTS).
...
# REFERENCES_SQL's crfn fallback (the LATERAL lookup on real_property_master.crfn)
# is unusably slow without an index on that column: the WoW / justfixwow dumps ship
# indexes on documentid/doctype/docdate/docamount but NOT crfn, so a full run
# seq-scans 17M rows per reference and appears to hang. Ensure it here so the step
# is self-provisioning on any database. IF NOT EXISTS => ~30s the first time, a no-op
# every run after. Needs index-create privilege on real_property_master (the loading
# user has it); resolves through search_path, same as every other unqualified read.
CRFN_INDEX_SQL = ("CREATE INDEX IF NOT EXISTS real_property_master_crfn_idx "
                  "ON real_property_master (crfn)")


def _ensure_crfn_index(conn) -> None:
    """Create the real_property_master(crfn) index the crfn reference-join needs (idempotent)."""
    with conn.cursor() as cur:
        cur.execute(CRFN_INDEX_SQL)
    conn.commit()
    print("  ensured index real_property_master_crfn_idx (crfn reference lookups)")
